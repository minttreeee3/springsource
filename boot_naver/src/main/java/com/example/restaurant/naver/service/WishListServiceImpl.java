...
public class WishListServiceImpl implements WishListService {
	...
	@Autowired
	private NaverClient naverClient;	//매퍼역할
	...
	public WishListDTO search(String query) {
		...
		if(res.getTotal() > 0) {
			...
			// 해당 가게 사진을 검색하기 위해서 제목을 추출 
			String imageQuery = localItem.getTitle().replaceAll("<[^>]*>", ""); //만약 꺽쇠 들어있으면 없애란 뜻
			...
			if(imageRes.getTotal() > 0) {
				...
				// 받은 정보들을 원하는 데이터만 추출해서 WishListDTO에 담기
