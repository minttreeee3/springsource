...
// 횡단관심사를 처리해 줄 클래스
// 횡단관심사를 언제 처리할 것인가?
// Before(메소드를 호출 전에), After returning(메소드 호출 이후에), After throwing(메소드 예외 발생 후) 
// After(메소드 호출 후 예외 발생 여부와 관계없이), Around(메소드 호출 이전과 이후 모두 적용) 

@Component("log")
@Aspect
public class LogAdvice {
	// execution() : 표현식 - 특정메소드만 advice함 
	// * : 리턴타입
	// com.spring.aop : 패키지명
	// Product : 클래스명
	// getInfo() : 메소드명
...
//	}
	...
	public void afterLog() {
		System.out.println("[공통로그] 비즈니스 로직 수행 후 호출(예외 발생여부와 관계없이 무조건 실행)");
	}
	
	@AfterThrowing(value = "execution(* com.spring.aop.Product.getInfo())")
	public void afterThrowingLog() {
		System.out.println("[공통로그] 비즈니스 로직 수행 후 호출(예외 발생시에만)");
	}
	
	@AfterReturning(value = "execution(* com.spring.aop.Product.getInfo())")
	public void afterReturnLog() {
		System.out.println("[공통로그] 비즈니스 로직 수행 후 호출(예외 발생 없을때에만)");
	}
